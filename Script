...
# Configurazione Faker per la generazione di dati casuali
faker = Faker()
Faker.seed(98765)
def generate_fake_data(records=10):
    ...
    data = {
        ...
        'Email': [faker.email() for _ in range(records)],
        'Telefono': [faker.phone_number() for _ in range(records)]
    }
...
def fetch_from_sql(db_name="sqlite:///database_utenti_nuovo.db"):
